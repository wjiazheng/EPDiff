...
class Mlp(nn.Module):
    def __init__(self,
        ...
        # embedding_dim_out = out_channels or in_channels
        # self.fc1 = nn.Linear(in_channels*8*8, hidden_dim)
        self.act = activation()
        # self.fc2 = nn.Linear(hidden_dim, embedding_dim_out*8*8)

    def forward(self, x):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        norm1 = nn.LayerNorm(x.shape[0]).to(device)
        s1 = x.shape[1]
        s2 = x.shape[2]
        s3 = x.shape[3]
        x = norm1(x.permute(1,2,3,0)).permute(3,0,1,2)
        x = x.view(x.shape[0],x.shape[1]*x.shape[2]*x.shape[3])
        h = x.shape[0]
        w = x.shape[1]
        fc = nn.Linear(w, h).to(device)
        x = fc(x)
        x = self.act(x)
        fc2 = nn.Linear(h, w).to(device)
        x = fc2(x)
        x = x.view(x.shape[0], s1, s2, s3)
        return x
...
class UNet(nn.Module):
    def __init__(self, c_in=4, c_out=4, img_size=128, time_dim=256, device="cuda"):
        ...
        # self.norm1 = nn.BatchNorm2d(512, eps=1e-3)
        self.mlp = Mlp()
        self.bot = DoubleConv(512,512)
        # self.connect_norm = nn.BatchNorm2d(512, eps=1e-3)
        self.mlp2 = Mlp()
    ...
    def unet_forward(self, x, t):
        ...
        x5 = self.bot1(x5)
        x5 = x5 + self.drop_path(self.mlp(x5))
        x5 = self.bot(x5)
        x5 = x5 + self.drop_path(self.mlp2(x5))
        x5 = self.bot3(x5)
